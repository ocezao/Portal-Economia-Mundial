...
export function UserDashboard() {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => {
    return (localStorage.getItem('pem_dashboard_view') as 'grid' | 'list') || 'grid';
  });
  ...
  useEffect(() => {
    const calculateStreak = () => {
      ...
      setStreak({
        ...
        longest: Math.max(currentStreak, parseInt(localStorage.getItem('pem_longest_streak') || '0')),
        lastReadDate: sorted[0]?.readAt || '',
        weeklyReads,
      });
      
      localStorage.setItem('pem_longest_streak', Math.max(currentStreak, parseInt(localStorage.getItem('pem_longest_streak') || '0')).toString());
    };
  ...
  const toggleViewMode = () => {
    ...
    setViewMode(newMode);
    localStorage.setItem('pem_dashboard_view', newMode);
  };
          ...
          <aside className="space-y-6">
            ...
            <section className="bg-gradient-to-br from-[#111111] to-[#333] text-white rounded-xl p-4 sm:p-6">
              <section className="flex items-center gap-3 mb-3">
                ...
                <h3 className="font-bold">Newsletter PEM</h3>
              </section>
              <p className="text-sm text-gray-400 mb-4">
                Receba análises exclusivas e as principais notícias no seu e-mail.
              </p>
              <Link to={ROUTES.app.configuracoes}>
