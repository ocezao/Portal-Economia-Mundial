...
import { createLocalUser, deleteLocalUser, listLocalUsers, updateLocalUser } from '@/lib/server/localAuth';
...
export async function POST(req: Request) {
  try {
    ...
    if (action === 'create_user') {
      ...
      if (!email || !password || !name) return json({ error: 'Nome, email e senha sao obrigatorios' }, 400);
    ...
    if (action === 'update_password') {
      ...
      if (email && isEmailConfigured()) {
        const tpl = accountPasswordUpdatedTemplate(name || email);
        await sendEmailSafe({
          to: email,
          subject: tpl.subject,
          html: tpl.html,
