...
export default function AdminDiagnosticoPage() {
  ...
  const checkStorageAccess = () => {
    const publicKey = '__cin_diag_public';
    const secureKey = '__cin_diag_secure';

    let localStorageOk = true;
    let sessionStorageOk = true;

    try {
      publicStorage.set(publicKey, 'ok');
      const val = publicStorage.get<string>(publicKey);
      publicStorage.remove(publicKey);
      localStorageOk = val === 'ok';
    } catch {
      localStorageOk = false;
    }

    try {
      secureStorage.set(secureKey, 'ok');
      const val = secureStorage.get<string>(secureKey);
      secureStorage.remove(secureKey);
      sessionStorageOk = val === 'ok';
    } catch {
      sessionStorageOk = false;
    }

    return { localStorageOk, sessionStorageOk };
  };
  ...
  const runDiagnostics = async () => {
    ...
    // 1. Verificar integridade do storage (localStorage e sessionStorage)
    try {
      const { localStorageOk, sessionStorageOk } = checkStorageAccess();
      
      const storageOk = localStorageOk && sessionStorageOk;
      ...
      checks.push({
        ...
        message: storageOk 
          ? 'localStorage e sessionStorage acessíveis' 
          : `Erro: ${!localStorageOk ? 'localStorage' : ''} ${!sessionStorageOk ? 'sessionStorage' : ''}`,
      });
    } catch {
      checks.push({
        id: 'storage',
    ...
    const registeredUsers = storage.get<unknown[]>('cin_registered_users') || [];
    ...
    const comments = storage.get<unknown[]>('cin_comments') || [];
